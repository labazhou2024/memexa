# Common developer tasks for the open demo. Run ``make help`` for the menu.

.PHONY: help install fmt lint test smoke pii-scan demo demo-ingest clean

help:
	@echo "Usage: make <target>"
	@echo ""
	@echo "Setup"
	@echo "  install         pip install -e .[dev]"
	@echo "  fmt             ruff format + black"
	@echo "  lint            ruff check + black --check"
	@echo ""
	@echo "Test"
	@echo "  test            run all tests (no backend required)"
	@echo "  smoke           run PII scan + tests + demo"
	@echo "  pii-scan        scan whole tree for residual PII tokens"
	@echo ""
	@echo "Demo"
	@echo "  demo            run the 30-second in-memory demo"
	@echo "  demo-ingest     build bundled demo cards without POSTing"
	@echo ""
	@echo "Cleanup"
	@echo "  clean           remove .venv, build artefacts, caches"

install:
	pip install -e ".[dev]"
	pre-commit install

fmt:
	ruff check --fix memexa tests
	ruff format memexa tests
	black memexa tests

lint:
	ruff check memexa tests
	black --check memexa tests

test:
	pytest -q

pii-scan:
	@bash scripts/pre-commit-pii-scan.sh || (echo "(running full-tree scan, not just staged)"; \
	    bash scripts/full_pii_scan.sh && echo "✅ no residual PII" || exit 1)

demo:
	python -m memexa.cli.main demo

demo-ingest:
	python -m examples.demo_dataset.ingest --dry-run

smoke: pii-scan test demo

clean:
	rm -rf .venv .pytest_cache .ruff_cache .mypy_cache htmlcov .coverage
	find . -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true
	find . -name "*.egg-info" -type d -exec rm -rf {} + 2>/dev/null || true
