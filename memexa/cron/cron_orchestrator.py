...
CLI:
  python -m memexa.core.cron_orchestrator list-drivers
  python -m memexa.core.cron_orchestrator run-incremental --driver diary
  python -m memexa.core.cron_orchestrator run-incremental --all
  python -m memexa.core.cron_orchestrator run-incremental --all --dry-run
  python -m memexa.core.cron_orchestrator validate-manifest
"""
...
def _load_manifest():
    from memexa.core.cron_manifest import load
    return load()
...
def dispatch_incremental(
    ...
    try:
        from memexa.core.win_job_subprocess import run_with_job_object
...
def _cli() -> int:
    p = argparse.ArgumentParser(
        prog="python -m memexa.core.cron_orchestrator",
