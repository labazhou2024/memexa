"""V5 WeChat backfill driver — cron-callable incremental pipeline.

Orchestrates the full v5 ingestion pipeline for WeChat:
  Step 1: Load cursor from data/backfill_v5_wechat_progress.json
  Step 2: Build new input batches via tools/v5_wechat_batch_builder.py
  Step 3: Identify pending batches (not yet extracted or posted)
...
Cursor file schema (data/backfill_v5_wechat_progress.json):
{
  "last_built_ts": 1715000000.0,   // Unix epoch; batch_builder --since value
...
CLI:
  python -m tools.backfill_v5_wechat_driver
  python -m tools.backfill_v5_wechat_driver --since 2026-05-01
  python -m tools.backfill_v5_wechat_driver --max-batches 10
  python -m tools.backfill_v5_wechat_driver --dry-run --verbose
  python -m tools.backfill_v5_wechat_driver --skip-build
  python -m tools.backfill_v5_wechat_driver --skip-post
...
from memexa.core._path_resolver import data_dir as _resolve_data_dir
_DATA = _resolve_data_dir()

_CURSOR_PATH = _DATA / "backfill_v5_wechat_progress.json"

_INPUT_BATCHES_DIR = _DATA / "l0_v5" / "input_batches"
_CARDS_DIR = _DATA / "l0_v5" / "work" / "cards_v2_wechat"
_POSTED_DIR = _DATA / "l0_v5" / "work" / "posted_v5_wechat"

_BATCH_BUILDER = _REPO / "ingestion" / "v5_wechat_batch_builder.py"
...
# 2026-05-11 v2: phase-split throughput maximization. l0_worker_serial already
# uses group-size=50 default → one swap_to_qwen → Phase A all N → one
# swap_to_gemma → Phase B all accepted → one swap back. So per-batch swap
# overhead is amortized across the whole batch. With cron_orchestrator timeout
# bumped from 1200s→2400s, can fit ~20 batches (Phase B ~30 min worst case).
# 2026-05-11 v3 (Phase 2.2): default 20→5. Phase 1 your-org backfill clears the
...
# in PG memory_full_v5, with 1h TTL. Resolves marker-vs-PG drift (§D.14):
# 1,853 wechat ghost markers no longer cause "real backlog hidden".
_SOURCE = "wechat"
...
def _load_cursor() -> dict:
    ...
    try:
        ...
        print(f"[backfill_v5_wechat_driver] WARN: cursor load failed ({exc}), using default",
              file=sys.stderr)
        return {
...
def _save_cursor(cur: dict) -> None:
    ...
    try:
        ...
        print(f"[backfill_v5_wechat_driver] WARN: cursor save failed: {exc}",
              file=sys.stderr)
...
def _list_pending_batches(
    ...
    - PG `memory_full_v5` has a card with this batch_id (authoritative truth) — NEW
    - posted_dir/<batch_id>.posted marker exists (local cache)
    - cards_dir/<batch_id>.json exists (extracted, awaiting POST)

    PG check uses memexa.core.pg_bid_cache with 1h TTL (~700ms uncached, ~0ms
    cached). On PG unreachable the cache returns last-known set (stale OK) or
    empty (degrade to marker-only check, same as pre-Phase-2.1 behaviour).

    Returns up to `limit` batch_ids (lexicographic order for determinism).
    """
    if not input_dir.exists():
        return []

    # Collect all batch directories from input
    ...
    # 2026-05-11 v3 (Phase 2.1): query PG for authoritative "already in graph"
    # set. Resolves ghost-marker drift (HANDOFF §D.14): 1,853 wechat ghost
    # markers wrongly hide real backlog; PG truth shows the real ~1,049 missed.
    if str(_REPO) not in sys.path:
        sys.path.insert(0, str(_REPO))
    try:
        from memexa.core.pg_bid_cache import query_pg_existing_bids
        pg_bids = query_pg_existing_bids(source)
    except Exception as exc:
        print(f"[backfill_v5_wechat_driver] WARN: pg_bid_cache fail ({exc}), "
    ...
    for bid in all_batch_ids:
        if bid in pg_bids:
            continue  # already in PG, marker accuracy irrelevant
...
def _emit_trace(event: str, payload: dict) -> None:
    """Emit a trace event via memexa.core.trace_sink if available; else stderr."""
    try:
        # Ensure memexa package importable
        if str(_REPO) not in sys.path:
            sys.path.insert(0, str(_REPO))
        from memexa.core.trace_sink import write_trace_event  # type: ignore
...
def _stage_build_batches(
    ...
    """Step 2: Run v5_wechat_batch_builder to build new input batches.

    Returns (n_new_batches, build_start_ts).
    ...
    if since_override:
        ...
        try:
            ...
            print(f"[backfill_v5_wechat_driver] WARN: invalid --since '{since_override}', "
                  "falling back to cursor", file=sys.stderr)
            since_ts = cursor["last_built_ts"]
    else:
        since_ts = cursor["last_built_ts"]

    # 2026-05-10 fix: v5_wechat_batch_builder.py CLI is --start/--end YYYY-MM-DD
    # (not --since epoch). Convert + add 1-day overlap upstream for safety.
    ...
    if verbose:
        print(f"[step2] building batches start={start_str} end={end_str}: {' '.join(cmd)}")

    res = _run_subprocess(cmd, timeout=600, verbose=verbose)
    if res["returncode"] != 0:
        print(f"[backfill_v5_wechat_driver] WARN: batch_builder exited {res['returncode']}",
              file=sys.stderr)
        if res["stderr"]:
...
def _stage_run_worker(
    ...
    """Step 4: Run extraction worker. mode=local (Mac dual-LLM) | api (your-org)."""
    if not pending:
    ...
    # Pass 2 does both Stage A (Qwen gatekeeper) + Stage B (Gemma extractor).
    done_dir = _DATA / "l0_v5" / "work" / "done_v2_wechat"
    done_dir.mkdir(parents=True, exist_ok=True)
    # 2026-05-12 (Step 3): --mode=api routes to your-org API worker; --mode=local
    # (default) keeps Mac dual-LLM. Both workers honor same CLI contract.
    ...
    if verbose:
        print(f"[step4] running Mac worker on {len(pending)} pending batches")
        print(f"  cmd: {' '.join(cmd)}")

    res = _run_subprocess(cmd, timeout=14400, verbose=verbose)
    if res["returncode"] != 0:
        print(f"[backfill_v5_wechat_driver] WARN: worker exited {res['returncode']} "
...
def _stage_post_cards(dry_run: bool, verbose: bool) -> dict:
    ...
    if verbose:
        print(f"[step5] posting cards to {_HINDSIGHT_BASE_URL}/{_HINDSIGHT_BANK}")
        print(f"  cmd: {' '.join(cmd)}")

    res = _run_subprocess(cmd, timeout=14400, verbose=verbose)
    if res["returncode"] != 0:
        print(f"[backfill_v5_wechat_driver] WARN: streaming_post exited {res['returncode']}",
              file=sys.stderr)
        if res["stderr"]:
            print(res["stderr"][:2000], file=sys.stderr)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="V5 WeChat backfill driver — incremental cron pipeline",
    )
    parser.add_argument(
        "--since",
        metavar="YYYY-MM-DD",
    ...
    if not args.skip_build:
        try:
            ...
            print(f"[backfill_v5_wechat_driver] ERROR in build stage: {exc}",
                  file=sys.stderr)
            any_failure = True
    else:
        if args.verbose:
    ...
    if args.verbose or args.dry_run:
        print(f"[step3] pending batches (limit={args.max_batches}): {len(pending)}")
    ...
    if pending:
        try:
            ...
            print(f"[backfill_v5_wechat_driver] ERROR in worker stage: {exc}",
                  file=sys.stderr)
            any_failure = True
    else:
        if args.verbose:
    ...
    if not args.skip_post:
        try:
            ...
            print(f"[backfill_v5_wechat_driver] ERROR in post stage: {exc}",
                  file=sys.stderr)
            any_failure = True
    else:
        if args.verbose:
    ...
    # Emit trace event
    _emit_trace("v5_wechat_driver_run", summary)
