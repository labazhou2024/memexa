...
Drives 3 independent batches in series (intentional — single shared cost cap):
  1. WeChat (memexa.extraction.wechat_batch_ingest.BatchIngestRunner)
  2. QQ     (memexa.extraction.qq_batch_ingest.QQBatchIngestRunner)
  3. Email  (memexa.extraction.email_batch_ingest.EmailBatchIngestRunner)
...
def _run_wechat(max_msgs: int, max_cost: float, max_daily: float,
                 dry_run: bool) -> dict:
    try:
        from memexa.extraction.wechat_batch_ingest import BatchIngestRunner
...
def _run_qq(max_msgs: int, max_cost: float, max_daily: float,
             dry_run: bool) -> dict:
    try:
        from memexa.extraction.qq_batch_ingest import QQBatchIngestRunner
    except Exception as e:
...
def _run_email(max_msgs: int, max_cost: float, max_daily: float,
                dry_run: bool) -> dict:
    try:
        from memexa.extraction.email_batch_ingest import EmailBatchIngestRunner
    except Exception as e:
...
def main() -> int:
    """CLI: `python -m memexa.extraction.chat_unified_batch_ingest [--dry-run]`."""
    import argparse
