...
def _has_specificity(text: str) -> bool:
    ...
    signals = [
        r"[\w/]+\.\w{1,4}",           # 文件路径 (e.g., memexa/main.py)
...
def format_output(matches: List[Tuple[KeywordRule, int]], prompt: str) -> str:
    ...
    if top_match.name == "autopilot":
        try:
            ...
            from memexa.core.task_router import (
...
def main():
    ...
    try:
        ...
        if matches:
            ...
            if top.name in ("persistent", "autopilot"):
                try:
                    ...
                    from memexa.core.persistent_mode import activate
            ...
            if top.name == "cancel":
                try:
                    ...
                    if _jr not in _sys.path:
                        _sys.path.insert(0, _jr)
                    from memexa.core.persistent_mode import deactivate
                    import os as _os2
        ...
        if os.environ.get("MEMEXA_CEO_FEEDBACK_HOOK", "1") == "1":
            try:
                from memexa.core.ceo_feedback import hook_user_prompt_submit
        ...
        if os.environ.get("MEMEXA_QUERY_TYPE_LOG", "1") == "1":
            try:
                ...
                if _jr not in _sys.path:
                    _sys.path.insert(0, _jr)
                from memexa.core.query_type_classifier import log_query_type
        ...
        if correction:
            ...
            try:
                from memexa.core.soft_signal_classifier import classify_soft_signal
        ...
        try:
            ...
            from memexa.core.pattern_extractor import smart_prime, format_prime_output
        ...
        if os.environ.get("MEMEXA_GRAPH_RETRIEVE", "1") != "0":
            try:
                from memexa.core.canonicalizer import (
                ...
                from memexa.core.graph_memory_v2 import query_entity
