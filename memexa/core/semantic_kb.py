...
def build_index(force: bool = False) -> int:
    ...
    if not is_enabled():
        return 0
    try:
        from memexa.core.pattern_extractor import load_all_patterns, _PATTERNS_FILE
    except Exception:
        return 0
...
def semantic_search_boosted(
    ...
    try:
        from memexa.core.canonicalizer import canonicalize_entity, _build_entity_map, _normalize
    except Exception:
        return cosine_hits[:top_k]
    ...
    if use_graph:
        try:
            ...
            from memexa.core.graph_memory_v2 import query_entity as _gq
...
def main():
    """CLI: python -m memexa.core.semantic_kb [build|search QUERY]"""
    import sys
    if len(sys.argv) < 2:
