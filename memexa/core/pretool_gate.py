...
def _respond(decision: str, reason: str = "", context: str = "", rule: str = "default", target: str = ""):
    ...
    try:
        ...
        from memexa.core._hook_utils import log_gate_decision  # noqa: E402
        log_gate_decision(
            gate="pretool_gate",
            rule=rule,
            decision=decision,
...
def _inline_prime(cmd: str, preferred_memory: str = "") -> str:
    ...
    hits_text = ""
    try:
        from memexa.core.pattern_extractor import prime as _prime
        if triggered_kws:
...
def _check_cost_budget_block(cmd: str) -> "str | None":
    ...
    return (
        ...
        "Run: python -m memexa.core.cost_meter summary "
        f"{tid}"
    )
...
#   - flag is written by memexa.core.sonnet_quarantine_counter when the daily
...
def _check_sonnet_quarantine_block(cmd: str) -> "str | None":
    ...
    return (
        ...
        "CEO must review + run "
        "`python -m memexa.core.sonnet_quarantine_counter clear`."
    )
...
def _check_memory_runaway_block(cmd: str) -> "str | None":
    ...
    return (
        ...
        "run `python -m memexa.core.memory_guardrail clear` to release the gate."
    )
...
def _rule14_owasp_env_inline(tool_name: str, tool_input: dict):
    ...
    if tokens[0].startswith("MEMEXA_GATES_OVERRIDE="):
        return ("deny",
                ...
                "(`python -m memexa.cli.gates_override mint`), not LLM-authored inline shell. "
...
def main():
    ...
    if tool_name == "Task" or tool_name == "Agent":
        ...
        if stype:
            try:
                from memexa.core.agent_stall_detector import pre_check as _stall_pre
    ...
    if tool_name == "Bash":
        ...
        # --------------------------------------------------------------
        try:
            from memexa.core import cmd_retry_tracker
        ...
        # by memexa.core.sonnet_quarantine_counter.increment().
        # --------------------------------------------------------------
        try:
        ...
        # memexa.core.memory_guardrail._write_flag(). Reference incident:
        ...
        if fp_norm.endswith("/memexa/data/env_allowlist.json") or \
           fp_norm.endswith("memexa/memexa/data/env_allowlist.json"):
            _respond(
                ...
                "Use: python -m memexa.core.ci_env_grep_gate add-allowed "
    ...
    if tool_name in ("Write", "Edit"):
        if Path(file_path).name == "autopilot_active.json":
            ...
            _respond(
                ...
                "python -m memexa.core.persistent_mode activate <desc> autopilot "
    ...
    if file_path.endswith(".py"):
        try:
            from memexa.core.retry_tracker import can_retry
                ...
                if spec.get("complexity") == "complex" and spec.get("status") == "in_progress":
                    ...
                    if "plan_approved" in criteria and not criteria["plan_approved"]:
                        ...
                        _respond("deny", (
                            ...
                            "Then: python -c \"from memexa.core.task_router import verify_criteria; "
...
def _check_plan_revision(file_path: str, version: int,
    ...
    try:
        ...
        try:
            from memexa.core.plan_spec import load_plan, _parse_plan_markdown
            from memexa.core.axis_lock import check_revision
