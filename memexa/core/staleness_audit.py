...
CLI
---
    python -m memexa.core.staleness_audit audit            # single pass, print summary
    python -m memexa.core.staleness_audit briefing         # generate + print path
    python -m memexa.core.staleness_audit metrics          # pretty status
    python -m memexa.core.staleness_audit apply <file>     # parse marks, call promote/reject
"""
...
# ---------------------------------------------------------------------------
try:
    from memexa.core.promotion_engine import promote, reject  # type: ignore
    from memexa.core.promotion_engine import _load_pending, _save_pending  # type: ignore
...
try:
    from memexa.core.session_heartbeat import count_ceo_active_days  # type: ignore
    from memexa.core.session_heartbeat import ceo_days_since as _sh_ceo_days_since  # type: ignore
    _has_sh_helper = True
except Exception:  # pragma: no cover
...
try:
    from memexa.core._pending_io import (
...
def generate_cto_briefing(
    ...
    lines: List[str] = [
        ...
        "> Then run: `python -m memexa.core.staleness_audit apply <this_file>`",
        "",
