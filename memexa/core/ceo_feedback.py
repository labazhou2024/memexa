...
2. **Explicit CLI** — `python -m memexa.core.ceo_feedback rate <N> [reason]`
...
def _scrub_reason(text: str) -> str:
    ...
    if not text:
        return ""
    try:
        from memexa.core.prompt_evolver import _sanitize_for_llm
...
def _cli() -> int:
    """Usage:
      python -m memexa.core.ceo_feedback rate <1-5> [reason]
      python -m memexa.core.ceo_feedback scan "<text>"   (test detection)
      python -m memexa.core.ceo_feedback stats            (summary)
    """
    if len(sys.argv) < 2:
