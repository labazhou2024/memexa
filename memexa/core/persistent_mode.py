...
# (not `python -m memexa.core.persistent_mode`), the `memexa` package is not on
# sys.path. Internal imports below (and via call into other modules) need it.
# Library imports (`from memexa.core import persistent_mode`) make this a harmless
...
def _is_spec_stale(spec_file) -> bool:
    ...
    from memexa.core._thresholds import SPEC_STALE_REGEN_SEC
    try:
...
def activate(
    ...
    if mode == "autopilot":
        try:
            from memexa.core._autopilot_flag import set_flag
...
def mark_completed(force: bool = False) -> bool:
    ...
    if state:
        ...
        if state.get("mode") == "autopilot":
            ...
            if not force:
                try:
                    from memexa.core.pattern_extractor import extract_from_session
            ...
            try:
                from memexa.core.auto_trigger import record_autopilot_completion
            ...
            try:
                from memexa.core._autopilot_flag import clear_flag
                clear_flag()
            except Exception:
                pass
            try:
            ...
            try:
                from memexa.core.pattern_extractor import credit_session_helpful
...
def refresh_stage(stage_name: str) -> Optional[dict]:
    ...
    if state and state.get("active"):
        ...
        try:
            from memexa.core._autopilot_flag import refresh_flag
            refresh_flag()
        except Exception:
            pass
        now = time.time()
        ...
        if stage_name == "stage_4_5_plan_retro":
            ...
            if tid:
                try:
                    from memexa.core.plan_retro_gate import (
