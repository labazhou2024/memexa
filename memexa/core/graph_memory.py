...
CLI (verified alive in dev):
  python -m memexa.core.graph_memory ensure-schema
  python -m memexa.core.graph_memory stats
  python -m memexa.core.graph_memory ingest <memory-file>
  python -m memexa.core.graph_memory query <entity>
  python -m memexa.core.graph_memory path <A> <B>
  python -m memexa.core.graph_memory recent [--hours N]
  python -m memexa.core.graph_memory backfill [--limit N] [--dry-run]
  python -m memexa.core.graph_memory invalidate <fact_id> <reason>
"""
...
from memexa.core._path_resolver import memory_dir as _default_memory_dir
...
def write_fact(
    ...
    try:
        from memexa.core.canonicalizer import canonicalize_entity as _canon_ent
    ...
    try:
        from memexa.core.predicate_semantics import normalize_predicate_canon
    ...
    try:
        from memexa.core.entity_kind import classify_entity as _ek
...
def _maybe_supersede(
    ...
    """
    try:
        from memexa.core.predicate_semantics import is_supersession_eligible
        if not (
...
def query_entity(query: str, limit: int = 20) -> List[FactRow]:
    ...
    if d is None:
        return []
    try:
        from memexa.core.canonicalizer import canonicalize_entity, _normalize
...
def query_path(entity_a: str, entity_b: str, max_hops: int = 3) -> List[List[Dict[str, Any]]]:
    ...
    if d is None:
        return []
    try:
        from memexa.core.canonicalizer import canonicalize_entity
...
def migrate_entity_kind(
    ...
    """
    from memexa.core.entity_kind import classify_entity
    from memexa.core._migration_errors import MigrationAmbiguousError
...
def audit_predicates() -> Dict[str, Any]:
    ...
    """
    from memexa.core.predicate_semantics import classify_predicate
    d = _get_driver()
    if d is None:
        return {"ok": False, "error": _driver_error}
    try:
...
def ingest_file(path: Path, chunk_size: int = 800,
    ...
    """
    from memexa.core.dual_llm_extractor import extract_single_llm
