...
调用方式:
  from memexa.core.task_router import classify_task, get_workflow
    ...
    TaskType.RESEARCH: Workflow(
        ...
        steps=[
            ...
            WorkflowStep("prime_kb", "加载知识库中的相关 pattern", agent=None,
                         verify="python -m memexa.core.pattern_extractor prime --keywords TASK_KEYWORDS"),
            ...
            WorkflowStep("extract_patterns", "从调研结果提取 pattern 到知识库", agent=None,
                         verify="python -m memexa.core.pattern_extractor stats"),
        ],
    ),
    ...
    TaskType.QUALITY: Workflow(
        ...
        steps=[
            ...
            WorkflowStep("bigloop", "执行完整 BigLoop Q0-Q7", agent=None,
                         verify="python -c \"from memexa.core.big_loop import run_big_loop_sync; print(run_big_loop_sync())\""),
...
# V2: security scan
from memexa.core.security_scanner import scan_directory, format_report
findings = scan_directory(Path('memexa/core'))
print(format_report(findings, 'memexa/core/'))

# V3: knowledge base prime
from memexa.core.pattern_extractor import prime, format_prime_output
...
```python
from memexa.core.task_router import verify_criteria
verify_criteria('CRITERION_ID', 'evidence text')
```

Check if all done:
```python
from memexa.core.task_router import check_all_criteria_met
...
```python
from memexa.core.task_router import verify_criteria, check_all_criteria_met
...
def generate_plan_prompt(task_type: TaskType, user_prompt: str) -> str:
    ...
    try:
        import os as _os
        from memexa.core.improvement_pattern_injector import (
    ...
    lines.append("```python")
    lines.append("from memexa.core.task_router import generate_task_spec, TaskType")
    ...
    lines.append("```python")
    lines.append("from memexa.core.task_router import verify_criteria, check_all_criteria_met")
