...
Called as:
  python -m memexa.core.mini_loop_runner run [--pre-commit] [--tid TID]
  python -m memexa.core.mini_loop_runner status
  python -m memexa.core.mini_loop_runner config-show
"""
...
# extracts this to ``memexa.core._thresholds`` so persistent_mode (REGEN
...
from memexa.core._thresholds import SPEC_STALE_PROBE_SEC
...
DEFAULT_CONFIG: Dict = {
    ...
    "probe_commands": [
        "python -m memexa.core.gates.integration_gate check {tid}",
        "python -m memexa.core.gates.hook_chain_probe {tid}",
    ],
}
...
def run_probes(
    ...
    for raw_cmd in probe_commands:
        ...
        try:
            from memexa.core.task_binding import propagate_to_subprocess
