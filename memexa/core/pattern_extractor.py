...
两种用法:
1. 提取: python -m memexa.core.pattern_extractor extract --source "code_review" --input "..."
2. 检索: python -m memexa.core.pattern_extractor prime --files "memexa/core/*.py" --keywords "hook"
...
def smart_prime(
    ...
    if not semantic_query.strip():
        return keyword_results
    try:
        from memexa.core.semantic_kb import semantic_search, is_enabled as sem_on
        if not sem_on():
            return keyword_results
...
def _extract_from_task_dir(task_id: Optional[str]) -> List[PatternEntry]:
    ...
    if not task_id:
        return []
    try:
        from memexa.core.task_dir_layout import task_dir as _tdir
        td = _tdir(task_id)
    except Exception:
        return []
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m memexa.core.pattern_extractor [extract|prime|stats]")
        return
