...
def check_auto_triggers() -> str:
    ...
    try:
        ...
        from memexa.core.auto_trigger import check_triggers, format_briefing
...
def _auto_submit_triggers(actions: list) -> list:
    ...
    for action in actions:
        ...
        if action_type == "auto_dream":
            ...
            try:
                from memexa.core.auto_dream import AutoDream
                # Run orient + gather + prune (skip consolidate which needs LLM)
                ad = AutoDream()
                episodes = ad.gather()
                prune_result = ad.prune()
                # Also prune expired patterns
                from memexa.core.pattern_extractor import prune_expired_patterns
            ...
            try:
                from memexa.core.auto_trigger import reset_mini_loop_counter
            ...
            action_items.append(
                ...
                f"Run: python -c \"from memexa.core.pattern_extractor import extract_from_session; "
...
def check_pending_approvals() -> str:
    ...
    """
    try:
        from memexa.core.approval_queue import get_queue_path
...
def _generate_ceo_briefing() -> str:
    ...
    try:
        sys.path.insert(0, str(_MEMEXA))
        from memexa.core.cto_briefing import generate_briefing
...
def check_resumable_task() -> str:
    ...
    try:
        sys.path.insert(0, str(_MEMEXA))
        from memexa.core.task_dir_layout import current_task_id, load_state
    except Exception:
        return ""
    try:
        tid = current_task_id()
        ...
        return (
            ...
            f"  Resume: python -m memexa.core.task_unit_scheduler resume {tid}"
        )
    except Exception:
        return ""
...
def check_crash_recovery() -> str:
    ...
    try:
        from memexa.core.event_bus import get_event_count
        ...
        return (
            ...
            f"Run manually: python -c \"from memexa.core.auto_dream import AutoDream; "
...
def _replay_last_commit_gates(threshold: int = 4) -> tuple:
    ...
    try:
        from memexa.core.trace_sink import read_traces
...
def main():
    ...
    try:
        from memexa.core.evolution_trigger import (
    ...
    if not _replay_ok:
        ...
        try:
            from memexa.core.approval_queue import submit_approval
            submit_approval(
                level="L2",
                ...
                proposal=(
                    "Run: python -m memexa.core.gates.integration_gate check "
    ...
    if "empty" not in approval and "No pending" not in approval:
        if verbose:
            ...
            print(f"[CEO_approvals] L3={l3} L2={l2} (run: python -m memexa.core.pending_approvals list)")
    ...
    if verbose:
        try:
            from memexa.core.evolution_dashboard import render_brief, is_enabled as dash_on
            if dash_on():
                dash = render_brief()
                if dash:
                    print(dash)
