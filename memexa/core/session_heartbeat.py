...
CLI
---
    python -m memexa.core.session_heartbeat emit
    ...
    python -m memexa.core.session_heartbeat ceo-active-days [N]
...
def emit(env: Optional[Mapping[str, str]] = None) -> Initiator:
    ...
    if initiator == "ceo":
        try:
            from memexa.core.memory_ingest_watcher import scan_with_timeout
...
def _iter_session_start_events(trace_file: Optional[Path] = None) -> Iterable[dict]:
    ...
    if trace_file is None:
        try:
            ...
            from memexa.core.trace_sink import _trace_file as _ts_trace_file
