...
CLI:
  python -m memexa.core.signal_bootstrap export     # JSON to stdout
  python -m memexa.core.signal_bootstrap grid       # interactive rating
  python -m memexa.core.signal_bootstrap readiness  # AC-15 status
"""
...
def export_candidates(
    ...
    """
    from memexa.core.pattern_extractor import load_all_patterns
...
def _load_patterns_from(patterns_file: Optional[Path]):
    ...
    if not patterns_file.exists():
        return []
    from memexa.core.pattern_extractor import PatternEntry
    entries = []
...
def record_bootstrap_rating(
    ...
    try:
        from memexa.core.ceo_feedback import record_feedback
...
def main() -> int:
    if len(sys.argv) < 2:
        print(
            "Usage: python -m memexa.core.signal_bootstrap "
