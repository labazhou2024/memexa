...
History:
- v1 (handcrafted): memexa.core.graph_memory.py 1285 LoC + 7 sibling modules = 3863 LoC
...
from memexa.core.hindsight_client import HindsightHttpClient, get_client
...
def _apply_anti_halluc_wrappers(facts: List[FactRow]) -> List[FactRow]:
    ...
    - EXISTING: invalidate post-filter via memexa.core.invalidate.list_tombstoned().
...
def query_entity(
    ...
    `memexa.core.memory_query.quick(query)` which provides:
    ...
    try:
        ...
        write_trace_event("graph_memory_v2_query_entity_called",
                          {"query_hash": hashlib.sha256(
                           ...
                           "deprecated": True,
                           "migrate_to": "memexa.core.memory_query.quick"})
    except Exception:
        pass
    ...
    if extracted_by is not None:
        ...
        from memexa.chat.metadata_builder import EXTRACTED_BY_CANONICAL
...
def write_fact(
    ...
    **kwargs: Any,
    ...
    try:
        from memexa.core.zh_coref import detect_zh_dense, inject_antecedents
...
def _main_query(entity: str, limit: int, timeout_s: float) -> int:
    ...
    print(
        ...
        "  → tell-me-about: python -m memexa.core.memory_query topic <topic>\n"
        "  → entity:        python -m memexa.core.memory_query quick <q>\n"
        "  → relationship:  python -m memexa.core.memory_query arc <name>\n"
        "  → time slice:    python -m memexa.core.memory_query timeline ...\n",
        file=_sys.stderr,
    )
...
def _main_backfill(rate: int, dry_run: bool, timeout_s: float) -> int:
    ...
    try:
        from memexa.core.hindsight_outbox import drain
