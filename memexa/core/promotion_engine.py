...
CLI
---
    python -m memexa.core.promotion_engine scan
        Find + enqueue every pattern that meets the thresholds.

    python -m memexa.core.promotion_engine list-pending
        Pretty-print the pending_approvals queue.

    python -m memexa.core.promotion_engine promote <pattern_id> <filename>
    ...
    python -m memexa.core.promotion_engine reject <pattern_id> <reason>
...
# Local imports
try:
    from memexa.core import pattern_extractor as _pe
except Exception:  # pragma: no cover - dependency is mandatory in prod
    _pe = None  # type: ignore[assignment]

try:
    from memexa.core.governance_middleware import pre_write_check  # type: ignore
...
try:
    from memexa.core.signal_bootstrap import (  # type: ignore
