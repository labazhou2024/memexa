...
Called by:
  ...
  3. CLI: python -m memexa.core.session_gate [commit-gate|session-end|check]
...
# inner `from memexa.core.X import Y` calls fail silently (caught by fail-soft
...
def _bench_gate(
    ...
    if bypass_token:
        try:
            from memexa.core._hook_utils import _verify_bench_bypass_token
    ...
    try:
        try:
            from memexa.core.bench_runner import run_benchmark, append_history
        except Exception as e:
...
def run_commit_gate(phase: Optional[int] = None) -> int:
    ...
    if _autopilot and py_files:
        ...
        if not _tid_check:
            print(
                ...
                "Run: python -m memexa.core.task_binding bind <task_id>  "
                "(see .claude/harness/tasks/ for active task ids). "
                "Or clear the flag to opt out: python -m memexa.core._autopilot_flag clear"
            )
    ...
    if py_files:
        try:
            ...
            lr_code = (
                ...
                "from memexa.core.local_reviewer import review_files\n"
            ...
            if len(_fail_open_reason) >= 80:
                ...
                if not _m:
                    ...
                    if not _ap.is_absolute():
                        ...
                        from memexa.core.task_dir_layout import _workspace_root as _wsr
    ...
    if active_tid:
        ...
        try:
            from memexa.core.plan_retro_gate import check_gate as _prg_check, \
            ...
            if _override_file.exists() and not _override_consumed:
                try:
                    ...
                    from memexa.core._gates_skip_budget import verify_override_token
        ...
        try:
            from memexa.core.gates.stage4_enforcement import check as _s4_check
    ...
    try:
        if active_tid:
            from memexa.core.failure_cluster_detector import list_live_findings
    ...
    # allow-silent: fail-soft observability path
    try:
        from memexa.core._hook_utils import log_gate_decision
        log_gate_decision(
            gate="session_gate",
            rule="commit_gate",
...
def _record_bootstrap_bypass() -> None:
    ...
    try:
        from memexa.core._atomic_state import atomic_update_json
...
def _rule7_depth_gate(task_id: str, code_lines: int) -> Tuple[bool, str]:
    ...
    try:
        from memexa.core.plan_spec import (
    ...
    if not health.healthy:
        ...
        if _auto:
            ...
            return (False,
                    ...
                    f"Run `python -m memexa.core.ac_verifier run {task_id}` first")
    ...
    if not health.healthy:
        ...
        if _auto:
            ...
            return (False,
                    ...
                    f"Run `python -m memexa.core.ac_verifier run {task_id}` then retry commit")
        return (True, f"data_source_{health.reason}_fail_open")
    # Zero-AC guard
    if len(spec.acceptance_criteria) < 1:
        from memexa.core.task_dir_layout import task_dir as _task_dir_fn
...
def _iter_findings(task_id: str):
    ...
    """
    try:
        from memexa.core.task_dir_layout import task_dir as _td_fn
        td = _td_fn(task_id)
    except Exception:
        return
...
def _check_mode_b_governance(active_tid: Optional[str],
    ...
    if n_main_session > 0:
        try:
            from memexa.core import ceo_approve as _ca
...
def _resolve_active_tid(
    ...
    if autopilot:
        try:
            from memexa.core._autopilot_flag import flag_info
            info = flag_info() or {}
...
def _phase_sentinel_path(task_id: str) -> Optional[Path]:
    ...
    """
    try:
        from memexa.core import plan_versioning
