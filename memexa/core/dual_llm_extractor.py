...
def _predicate_match_score(preds_a: List[str], preds_b: List[str]) -> float:
    ...
    try:
        from memexa.core.canonicalizer import canonicalize_predicate
    ...
    try:
        from memexa.core.graphiti_client import get_embedder
        model = get_embedder()
        if model is None:
            return exact_ratio
...
def _canonicalize_facts(facts: List[Fact]) -> List[Fact]:
    try:
        from memexa.core.canonicalizer import canonicalize_fact
    except Exception:
        for f in facts:
...
def _real_llm_call(model: str, chunk: str) -> List[Dict[str, Any]]:
    ...
    try:
        from memexa.core.llm_provider import call_llm, LLMError, ProviderUnavailable
    except Exception as e:
