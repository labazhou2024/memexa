...
def reset_reserves(module: Optional[str] = None) -> Dict:
    ...
    Usage: `python -m memexa.core.llm_budget reset-reserves [module]`
    """
    try:
        lock = _acquire()
    except ImportError:
...
def _cli(argv):
    """arch_v2 §F/§G playbook CLI: `python -m memexa.core.llm_budget status`."""
    ...
    if cmd == "reset-reserves":
        ...
        return 0
    print("usage: python -m memexa.core.llm_budget [status|reset-reserves [module]]", file=_sys.stderr)
    return 1
