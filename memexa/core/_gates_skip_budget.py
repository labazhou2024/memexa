...
CLI:
  python -m memexa.core._gates_skip_budget {show-budget|status}
"""
...
def _data_dir() -> Path:
    ...
    try:
        from memexa.core._paths import DATA_DIR
        return DATA_DIR
    except ImportError:
...
def _load_hmac_key() -> bytes:
    ...
    if not kf.exists():
        raise RuntimeError(
            f"HMAC key file missing; run: python -m memexa.cli.gates_override init-key"
            f" (expected at {kf})"
        )
    try:
        key = kf.read_bytes()
