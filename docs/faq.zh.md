...
(`memexa/ingestion/v5_wechat_batch_builder.py` 展示目标格式)。
...
1. 写 builder 把原始 export 转成规范 envelope schema。用
   `memexa/ingestion/v5_email_batch_builder.py` 当模板。
2. 写 driver 把 builder 接入 6 小时循环。用
   `memexa/drivers/backfill_v5_email_driver.py` 当模板。
3. 在 `data/cron_manifest.yaml` 注册 driver。
4. 跑 `python -m memexa.core.cron_orchestrator validate-manifest`。
5. 提 PR。
...
然后跑 `python -m memexa.core.pg_bid_cache all --force-refresh` 让 driver
