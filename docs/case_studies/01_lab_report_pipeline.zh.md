...
> 提交一份书面交付物。你不记得具体要求。**你的 memex bank 记得**。
>
...
对 memex 来说两者一样: **deadline + 交付物 + 散落在 4-6 source 的 context**。
...
边界跟动作一样重要。memex 是查询系统, 不是自主 agent。**做事**这一步
归你。
...
这个模式在每个 memex 工作流里都会出现。一旦内化, 14 个子命令塌缩成
一个心智模型:
...
| Step | 占位 | 你的值 |
|---|---|---|
| Step 1 | `memex pending` 里中期报告那行 | 你错过的那个交付物 |
...
- **对手方没明确表达过要求** — 你得先去问。memex 不会凭空造出从未表达
  过的要求。
