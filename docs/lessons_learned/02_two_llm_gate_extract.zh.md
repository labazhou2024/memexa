...
- `src/extraction/pass1_prompt.py` — gatekeeper prompt 模板
- `src/extraction/pass2_prompt.py` — extractor prompt + V2 envelope spec
- `src/extraction/l0_worker_serial.py` — 单笔记本带 2 次模型 swap 的串行
  worker
- `src/extraction/l0_worker_api.py` — 两个 OpenAI-compatible endpoint
