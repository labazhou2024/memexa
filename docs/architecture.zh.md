...
```
   ...
   (摄入)        │   src/ingestion/v5_*_batch_builder.py  │
                │   + src/extraction/qq/*                 │
...
Schema 规范在 `src/extraction/pass2_prompt.py` 是单一真值源。
...
`src/core/pg_bid_cache.py` 在 PostgreSQL 真值上加 1 小时 LRU。Driver 优先
...
`src/cron/cron_orchestrator.py` 掌管 6 小时增量循环:
...
`src/dashboard/sys_monitor` 是 FastAPI + vanilla-JS dashboard, 2 秒 tick
轮询 live state:
...
- Stage D 500/422 → 指数退避 (0/30/300/1800/14400 秒) 最多 5 次重试;
  最终失败写 `.dead`。
- Hindsight daemon 不可达 → 若设了 `MEMEX_HINDSIGHT_FALLBACK_URL` 则回退;
  否则错误传给调用方。
