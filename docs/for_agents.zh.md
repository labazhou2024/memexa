...
> **本文是什么**: 一份协议文档。memexa 是为 AI agent 设计的查询系统 ——
...
memexa 的 14 个子命令看起来是 14 种查图谱的方式。它们实际是一个小协议,
有可预测的失败模式。大多数 agent 在 memexa 上失败都是这 5-6 种:
...
client 端 post-filter。memexa 内部 helper `_post_filter()` 做这事;
直接调 API 时自己复制。
...
对 legacy bank, 传空 tag。memexa 的 bank wrapper 自动处理; 直接调 API
要自己处理。
...
```bash
memexa person "Y" --window-days 30        # 指令 + commitment
memexa arc "Y" --max-cards 60             # 关系弧
memexa quick "Y <最近关键词>" --max-k 15   # 最近 7 天
```
...
| 5 | `topic` 60s 超时 | 默认 `MEMEXA_HINDSIGHT_TIMEOUT=180` 够; 仍超时 = daemon 冷启 BGE-M3 | 30s 后重试一次 |
| 6 | Windows 上 UnicodeEncodeError | 终端是 GBK | 设 `PYTHONIOENCODING=utf-8` |
| 7 | 已知主题查询返 0 卡 | bank 错 | 检查 `MEMEXA_HINDSIGHT_BANK`; 默认 `memory_full_v5` |
...
- **生成用户没说过的文本** — `reflect` 综合现有卡; 不编造。用户问
  "我会怎么跟 X 说" 是 roleplay 请求, 不是 memexa 查询
- **写入用户刚刚打的新事实** — memexa 从 6 个 source 流走 cron 摄入。
  没有 `memexa remember "..."` 写 API (v0.x 可能加; 现在没)
- **排序用户没目睹的事件** — memexa 数据是用户的 exhaust。用户不在场,
  就没卡
