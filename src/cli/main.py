"""memex unified CLI dispatcher.

Subcommands:
  init       Scaffold ``~/.memex/`` config directory with example files.
  version    Print memex version + key dependency versions.
  config     Print resolved configuration (env vars + yaml files).
  query      Delegate to :mod:`src.core.memory_query` (14 subcommands).
  doctor     Run a self-diagnostic against the configured backend.

The classic ``memex-query`` console-script is an alias for ``memex query``
...
# when the user runs ``memex quick "X"`` instead of ``memex query quick "X"``.
_QUERY_SUBCMDS = {
    "quick",
    "topic",
    "arc",
...
def _cmd_version(_args: argparse.Namespace) -> int:
    """Print memex + Python + key-dep versions."""
    try:
        from src import __version__ as memex_version
    except Exception:
        memex_version = "unknown"

    print(f"memex  {memex_version}")
...
def _cmd_init(args: argparse.Namespace) -> int:
    """Create ``~/.memex/`` with example config files.

    Resolution order matches ``src.core._path_resolver``:
      1. ``--target`` flag
      2. ``MEMEX_CONFIG_DIR`` env var
      3. ``~/.memex``
    """
    target = (
        Path(args.target).expanduser()
        if args.target
        else Path(os.environ.get("MEMEX_CONFIG_DIR", str(Path.home() / ".memex"))).expanduser()
    )
    ...
    print("  6. Query:           memex quick \"test\"")
    return 0


def _cmd_config(_args: argparse.Namespace) -> int:
    """Print resolved configuration (env vars + file paths)."""
    print("=== memex configuration ===")
    print()
    print("Environment variables (MEMEX_*):")
    env_vars = sorted(k for k in os.environ if k.startswith("MEMEX_"))
    if env_vars:
        for k in env_vars:
            v = os.environ[k]
    ...
    print()
    print("Resolved paths (via src.core._path_resolver):")
    try:
        from src.core._path_resolver import (
            workspace_root,
            memory_dir,
            data_dir,
            logs_dir,
    ...
    print()
    print("Backend:")
    url = os.environ.get("MEMEX_HINDSIGHT_URL", "http://127.0.0.1:8888")
...
def _cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    url = os.environ.get("MEMEX_HINDSIGHT_URL", "http://127.0.0.1:8888")
    fallback_url = os.environ.get("MEMEX_HINDSIGHT_FALLBACK_URL", "").strip()
    bank = os.environ.get("MEMEX_HINDSIGHT_BANK", "memory_full_v5")
    ...
    try:
        ...
        print("[fail] httpx not installed — run: pip install memex")
        return 2
    ...
    if not primary_ok:
        ...
        if rc != 0:
            ...
            print("  - Set MEMEX_HINDSIGHT_URL=http://your-host:8888 in ~/.memex/.env")
            return rc
    ...
    for name in ("aliases.yaml", "identity.yaml"):
        ...
        if p.exists():
            ...
            print(f"[warn] {name} missing — run `memex init`")
...
def _probe_llm(httpx_module, *, role: str) -> int:
    ...
    differs.
    """
    base = os.environ.get("MEMEX_REMOTE_LLM_BASE_URL", "").rstrip("/")
    key = os.environ.get("MEMEX_REMOTE_LLM_API_KEY", "")
    model = os.environ.get(
        "MEMEX_REMOTE_LLM_GATE_MODEL" if role == "gate" else "MEMEX_REMOTE_LLM_EXTRACT_MODEL",
        "",
    )
    if not (base and model):
        print(f"[skip] LLM/{role}: MEMEX_REMOTE_LLM_BASE_URL or model not set")
        return 0
    try:
        r = httpx_module.post(
...
def _cmd_query(args: argparse.Namespace, remainder: List[str]) -> int:
    """Delegate to :func:`src.core.memory_query._cli`."""
    try:
        from src.core import memory_query  # type: ignore
    except Exception as e:
        print(f"[fail] cannot import src.core.memory_query: {e}", file=sys.stderr)
        return 1
    # Forward as if invoked via ``python -m src.core.memory_query <subcmd> ...``
    argv = ["src.core.memory_query"] + remainder
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="memex",
        description="memex — self-hosted Chinese personal memory graph",
    )
    p.add_argument(
        "--version",
        action="store_true",
    ...
    sp_version = sub.add_parser("version", help="print memex + dep versions")
    sp_version.set_defaults(func=_cmd_version)

    sp_init = sub.add_parser(
        "init",
        help="scaffold ~/.memex/ config directory",
    )
    sp_init.add_argument(
        "--target",
        type=str,
        default=None,
        help="config dir (default: $MEMEX_CONFIG_DIR or ~/.memex)",
    )
    sp_init.add_argument(
        "--force",
        action="store_true",
    ...
    # `memex query <subcmd>` proxies to memory_query
...
def main(argv: Optional[List[str]] = None) -> int:
    """Entry point installed as ``memex`` and ``memex-query``.
    ...
    # Back-compat: `memex quick "X"` (no `query` prefix) -> delegate to memory_query
    ...
    if args.cmd is None:
        parser.print_help()
        print()
        print("Hint: try `memex init` to scaffold config, or `memex quick \"X\"` for a query.")
        return 0
