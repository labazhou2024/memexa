...
Resume
------
python -m src.core.autopilot_checkpoint resume <tid>
...
def _load_persistent_mode_state() -> Optional[Dict[str, Any]]:
    try:
        from src.core.persistent_mode import _load_state
        return _load_state()
    except Exception:
        return None
...
def _load_scheduler_state(task_id: str) -> Optional[Dict[str, Any]]:
    try:
        from src.core.task_dir_layout import load_state
...
def resume(task_id: str, ckpt_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(pm_state, dict):
        try:
            from src.core.persistent_mode import _save_state
    ...
    if isinstance(sch_state, dict):
        try:
            from src.core.task_dir_layout import update_state
