"""src.core.memory_query — 单一图记忆查询入口 (Phase 2.3, 2026-05-06).
...
def _resolve_query_log_path() -> Path:
    ...
    try:
        from src.core._path_resolver import data_dir
...
def _get_client():
    global _client
    if _client is None:
        from src.core.hindsight_client import HindsightHttpClient
...
def _exclude_invalidated(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Reuse src.core.invalidate.list_tombstoned() to identify chunk_ids
...
def quick(
    ...
    *,
    ...
    canon_tags = None
    try:
        from src.core import identity_resolver as _idr
...
def summary(
    ...
    *,
    ...
    try:
        from src.extraction.ustc_llm_client import get_client as _gc
        client = _gc()
...
def _cli(argv: List[str]) -> int:
    ...
    try:
        ...
        pass
    p = argparse.ArgumentParser(prog="src.core.memory_query")
