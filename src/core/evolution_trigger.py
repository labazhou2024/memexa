...
def _is_eligible_agent_role(role: str) -> bool:
    ...
    try:
        from src.core.prompt_evolver import _is_safe_agent_name
        try:
            from src.core.prompt_evolver import _get_eligible_agents
            eligible = _get_eligible_agents()
        except ImportError:
            from src.core.prompt_evolver import _ELIGIBLE_AGENTS as eligible  # type: ignore
    except Exception:
...
def _evolve_one_agent(agent_name: str) -> dict:
    ...
    try:
        from src.core.prompt_evolver import MIN_OUTCOMES
    except Exception:
        MIN_OUTCOMES = 5
    ...
    try:
        import asyncio
        from src.core.prompt_evolver import get_prompt_evolver
...
def check_and_trigger() -> dict:
    ...
    try:
        ...
        try:
            from src.core.prompt_evolver import (
                is_agent_eligible,
                _ELIGIBLE_AGENTS,
            )
        except Exception as e:
...
def main():
    """CLI: python -m src.core.evolution_trigger [status|increment|check]"""
    import sys
    if len(sys.argv) < 2:
