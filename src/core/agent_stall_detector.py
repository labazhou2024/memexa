...
Manual reset: `python -m src.core.agent_stall_detector clear`.
"""
...
def pre_check(subagent_type: str) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if age > STALL_FLAG_TTL_SEC:
            ...
            from src.core._safe_fs import safe_unlink
        ...
        reason = (
            ...
            f"src.core.agent_stall_detector clear {sanitized}`."
        )
        return False, reason
    except Exception as e:
...
def _cli():
    args = sys.argv[1:]
    if not args:
        print("usage: python -m src.core.agent_stall_detector "
