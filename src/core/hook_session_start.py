...
CLI:
    python -m src.core.hook_session_start          # invoked by hook
    python -m src.core.hook_session_start --check  # status only, no spawn
    python -m src.core.hook_session_start --force-dream  # force trigger
"""
...
def _trigger_auto_dream() -> Dict[str, Any]:
    ...
    try:
        cmd = [sys.executable, "-m", "src.core.auto_dream", "consolidate"]
