...
def main() -> None:
    ...
    try:
        from src.core.mini_loop_runner import run_probes
    ...
    if result.get("any_failed"):
        ...
        reason = (
            ...
            f"`python -m src.core.mini_loop_runner status` then re-commit. "
            f"To override (NOT recommended): mint an HMAC override token via "
            f"`python -m src.cli.gates_override mint --reason '...'`. "
