...
CLI:
  python -m src.core.ac_verifier run <task_id> [--ac <AC_ID>] [--timeout 60]
  python -m src.core.ac_verifier status <task_id>
"""
...
# (e.g. monkeypatch.setattr("src.core.ac_verifier.task_dir", ...)).
try:
...
try:
    from src.core.plan_spec import get_latest, load_evidence
except Exception:  # pragma: no cover
...
def _trace_verified(task_id: str, ac_id: str, duration_ms: int) -> None:
    ...
    try:
        from src.core.failure_cluster_detector import clear_live_finding
...
def _trace_failed(task_id: str, ac_id: str, reason: str) -> None:
    ...
    try:
        from src.core.failure_cluster_detector import report_live_finding
...
def _cli(argv) -> int:
    ...
    if getattr(args, "task_dir_arg", None) is not None:
        ...
        import src.core.ac_verifier as _self
    ...
    if args.subcmd == "run":
        ...
        if args.ac:
            ...
            if args.inline_cmd:
                ...
                from src.core.plan_spec import get_latest
