...
set -e
cd "/c/Users/<USERNAME>/<WORKSPACE>/memex"
...
scp -q "remote-server:/tmp/memex_l0_round2/cards_v2/*.json" \
...
scp -q "remote-server:/tmp/memex_l0_round2/pass2_done/*.done" \
...
if [ "$PULLED" -gt 0 ]; then
  ...
  ssh -q remote-server "cd /tmp/memex_l0_round2/cards_v2 && \
  ...
  ssh -q remote-server "for f in /tmp/memex_l0_round2/cards_v2/*.json; do
    base=\$(basename \"\$f\")
  ...
  for win_card in data/l0_v5/work/round2_cards/*.json; do
    ...
    ssh -q remote-server "rm -f /tmp/memex_l0_round2/cards_v2/$bn" 2>/dev/null || true
  done
fi

# 4) <your-institution> remaining state
<your-institution>_STATE=$(ssh -q remote-server "echo cards=\$(ls /tmp/memex_l0_round2/cards_v2/*.json 2>/dev/null | wc -l) done=\$(ls /tmp/memex_l0_round2/pass2_done/*.done 2>/dev/null | wc -l) prompt_remain=\$(ls /tmp/memex_l0_round2/round2_input/*.json 2>/dev/null | wc -l) worker_alive=\$(pgrep -af l0_worker_v2_ustc | grep -v grep | wc -l)" 2>/dev/null)
...
if echo "$<your-institution>_STATE" | grep -q "prompt_remain=0" && echo "$<your-institution>_STATE" | grep -q "worker_alive=0"; then
  ...
  # final pull sweep
  scp -q "remote-server:/tmp/memex_l0_round2/cards_v2/*.json" data/l0_v5/work/round2_cards/ 2>/dev/null || true
  scp -q "remote-server:/tmp/memex_l0_round2/pass2_done/*.done" data/l0_v5/work/round2_done/ 2>/dev/null || true
  # final ssh-clean
  ssh -q remote-server "rm -rf /tmp/memex_l0_round2/{cards_v2,pass2_done,logs}/*; \
    rmdir /tmp/memex_l0_round2/round2_input 2>/dev/null || true" 2>/dev/null || true
