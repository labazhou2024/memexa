...
Serial lock: only ONE worker process may talk to :18081 at a time.
  Uses src.extraction.mlx_lm_serial_lock if available; falls back to no-op
...
# src.extraction.mlx_lm_serial_lock is written by parallel agent A1.
# If it doesn't exist yet, fall back to no-op stubs so this file imports clean.
try:
    from src.extraction.mlx_lm_serial_lock import acquire as _lock_acquire
    from src.extraction.mlx_lm_serial_lock import release as _lock_release
    _SERIAL_LOCK_AVAILABLE = True
except ImportError:
    warnings.warn(
        "[mac_worker] src.extraction.mlx_lm_serial_lock not found — "
...
def main(argv: Optional[List[str]] = None) -> int:  # noqa: C901
    ...
    if not _SERIAL_LOCK_AVAILABLE:
        logger.warning(
            ...
            "protection on Gemma-31B :18081. Install src.extraction.mlx_lm_serial_lock."
        )
    ...
    if args.rag_enabled and args.which_pass == 2:
        ...
        try:
            ...
            from src.core.identity_manifest import ManifestStore  # type: ignore[import]
