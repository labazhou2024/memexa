...
Usage:
  python -m src.extraction.batch_chat_extract --since-days 1 --max-batches 5
  python -m src.extraction.batch_chat_extract --since-days 2 --no-enqueue --json
"""
...
def extract_batch_paired(batch: ChatBatch,
    ...
    if classification is None:
        from src.extraction.batch_classifier import classify_batch, resolve_routing
        classification = classify_batch(batch)
    else:
        from src.extraction.batch_classifier import resolve_routing
    routing_key = resolve_routing(classification)

    # Layer C prompt template
    from src.extraction.batch_prompts import get_prompt
    ...
    if memory_summary:
        from src.extraction.chat_room_memory_summary import is_duplicate_fact
...
def main(argv=None) -> int:
    ...
    if not args.no_lifecycle and not args.dry_run:
        try:
            from src.core import mlx_lifecycle
    ...
    from src.extraction.batch_classifier import classify_batch
    from src.extraction.chat_room_memory_summary import get_chat_room_summary
    from src.core.episode_chain_builder import build_episodes
    ...
    if not args.no_lifecycle and not args.dry_run:
        try:
            from src.core import arbiter_27b_inline
