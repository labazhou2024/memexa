...
def _read_inbox_for_account(account: str, since_uid: int = 0,
    ...
    Skill resolution order:
      1. src.extraction.email_read.read_inbox(account, since_uid, max_count)
         (preferred — uses local cache + IMAP fallback)
      2. fallback: empty list (skill not available → no_skill)
    """
    try:
        from src.extraction.email_read import read_inbox
    except Exception:
        return []
...
class EmailBatchIngestRunner:
    ...
    def _extract_and_enqueue(self, msgs: list[dict],
        ...
        """
        try:
            from src.extraction.chat_extract_local import extract_msgs
            from src.extraction.chat_class_denylist import classify_drop_reason
            from src.extraction.consent_gate import evaluate_consent
        except Exception:
            return 0, 0.0
...
def main() -> int:
    """CLI entry: `python -m src.extraction.email_batch_ingest [--dry-run]`."""
    import argparse
